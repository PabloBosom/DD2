...
begin
  ...
  begin
    if nRst = '0' then
      ...
      fin_tx         <= '1';                                  
      tx_ok          <= '0';                                   
      nWR_op         <= '0'; --Tipo de operacion
      nWR            <= '0';  --Si hay que escribir(0) o leer (1)      
      fin_byte       <= '0';
  ...
  reset_SDA <= ini         when estado = libre           else     -- Condicion de start  (ini cuando estado = libre)
               ...
               '0';
                       ...
                       -- ******************************: ejercicio 8
  ACK_lectura <=      (nWR) and (not last_byte)   when estado = ACK          -- en una operacion de lectura deben asentirse todos los bytes leidos --Muchas dudad con esta
                         else '0';                                         -- menos el ultimo

                       -- ******************************: ejercicio 7
  preset_SDA <=    ena_stop_i2c when estado = stop else '0';       -- Condición de stop(segnalizacion de gen_SCL cuando estado = stop)
                       ...
                       -- ******************************: ejercicio 5
  desplaza_reg_out_SDA <=   ena_out_SDA when estado= tx_byte else '0';    -- Se desplaza el dato cuando lo indica la segnal de gen_SCL                         
  ...
  -- Control del registro de entrada:
                       -- ******************************: ejercicio 4
  leer_bit_SDA <=    ena_in_SDA when estado /= ACK else '0';          -- Se capturan todos los valores menos el ACK
