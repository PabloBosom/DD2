  ...
  begin
    if nRst'event and nRst = '0' then
      ...
      if tic_1s = '1' and info = 0 and info_T1 = 0 and (horas&minutos&segundos) /= 0 and programado = '0' then
        ...
        report "Error: El reloj no esta avanzando correctamente ya que el valor del tiempo anterior es distinto al nuevo"  -- TEXTO PARA SER MOFIFICADO CON UN MENSAJE MAS EXPLICATIVO
        severity error;
        ...
        report "Error: La hora no ha reiniciado correctamente y se ha sobrepasado"  -- TEXTO PARA SER MOFIFICADO CON UN MENSAJE MAS EXPLICATIVO
        severity error;


      elsif info_T1 /= 0 then
        assert segundos = 0
        report "Error: Error en el modo programacion lo segundos siguen contando"  -- TEXTO PARA SER MOFIFICADO CON UN MENSAJE MAS EXPLICATIVO
        severity error;
      ...
      if ((pulso_largo_T1 = '1' and cmd_tecla_T1 = X"C" and tic_025s_T1 = '1') or 
        ...
        if info_T1 = 1 then
		  if minutos /= 0 then -- si minutos no es "00"
			...
			if minutos(3 downto 0) /= 0 then  -- Si minutos no es "X0"s
				...
				report "Error en incremento de minutos por no incrementacion de las horas "
				severity error;
