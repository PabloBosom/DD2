...
esperar_hora(horas, minutos, AM_PM, clk, '1', X"11"&X"57");
...
--	esperar_hora(horas, minutos, AM_PM, clk, '0', X"11"&X"59");
	...
	entrar_modo_prog(pulso_largo,cmd_tecla,clk,10);
	wait until clk'event and clk = '1';
	wait until clk'event and clk = '1';
	...
	wait until clk'event and clk = '1';
	time_out(clk);	
--Entramos en modo programacion y poenmos la hora que queramos
    for i in 1 to 9 loop
       wait until clk'event and clk = '1';
    end loop;
entrar_modo_prog(pulso_largo,cmd_tecla,clk,10);
...
--Programamos la hora de uno en uno y comprobamos que avanza bien en 12 hroas
	programar_hora_inc_corto(ena_cmd,cmd_tecla,horas,minutos,AM_PM,clk,'0',X"11"&X"50");
	wait until clk'event and clk = '1';
	wait until clk'event and clk = '1';
	fin_prog(ena_cmd,cmd_tecla,clk);

	esperar_hora(horas, minutos, AM_PM, clk, '0', X"13"&X"57");

--Programamos la hora de mucho en mucho  y comprobamos que avanza bien 12 hroas
  for i in 1 to 9 loop
       wait until clk'event and clk = '1';
    end loop;
entrar_modo_prog(pulso_largo,cmd_tecla,clk,10);
	...
	programar_hora_inc_largo(pulso_largo,ena_cmd,cmd_tecla,horas,minutos,AM_PM,clk,'0',X"11"&X"50");
	wait until clk'event and clk = '1';
	wait until clk'event and clk = '1';
	fin_prog(ena_cmd,cmd_tecla,clk);
