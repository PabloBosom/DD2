...
port(clk:           in     std_logic;
     nRst:          in     std_logic;
     tic_025s:      in     std_logic;				-- TIC de 0.25 segundos
     tic_1s:        in     std_logic;				-- TIC de 1 segundo
     ena_cmd:       in     std_logic;				-- Pulsacion breve
     cmd_tecla:     in     std_logic_vector(3 downto 0);	-- Indentifica Tecla pulsada
     pulso_largo:   in     std_logic;				-- Pulsacion larga
     modo:          in     std_logic;
     ...
     info:          buffer std_logic_vector(1 downto 0));	-- Identifica el campo modificado
...
begin
  ...
  begin
    if nRst = '0' then
      ...
      if ena_cmd = '1' and time_out = '1' and estado = reposo then	-- Cambiamos "or" por "and"
        cnt <= "001";
  ...
  campo_valido <=  '1' when (estado /= horas)  and                dato_campo < X"60" and dato_campo(3 downto 0) < X"10" else
                   '1' when (estado  = horas)  and modo = '1' and dato_campo < X"24" and dato_campo(3 downto 0) < X"10" else
                   '1' when (estado  = horas)  and modo = '0' and dato_campo < X"12" and dato_campo(3 downto 0) < X"10" else
                   '0';
  ...
  begin
    if nRst = '0' then
	  ...
	  -- Vamos al modo normal si salimos de programacion por timeout o por pulsacion breve de 'A'
      if comando = fin_programacion or time_out = '1' then
        ...
        case estado is
          when reposo => 
		-- cuando esta en el modo reposo esta contando
  ...
  -- Generacion de senales de salida para la modificacion de la hora
