-- Esto es el top de la jerarquia, que se divide en el control y en el contador
...
U_0: entity work.ctrl_reloj(rtl)		-- RTL siempre que sea bloques sencillos (contadores, etc)
     port map(clk => clk,
              nRst => nRst,
              tic_025s => tic_025s,
              tic_1s => tic_1s,
...
U_1: entity work.cnt_reloj(estructural)		-- ESTRUCTURAL cuando esté compuesto por distintos bloques
     port map(clk => clk,
              nRst => nRst,
              tic_1s => tic_1s,
              ena_reloj => ena_reloj,
